                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="col-xs-12 col-md-12 input-group">
                            <select name="StudentID" class="form-control strip-tags" id="StudentID" required >
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="col-xs-12 col-md-12 input-group">
                            <select onchange="teacherWiseTask()" name="TeacherID" class="form-control strip-tags" id="TeacherID" required>
                                {{--<option value="">--Select--</option>--}}
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="col-xs-12 col-md-12 input-group">
                            <select onchange="taskWiseAssignmentList()" name="TaskID" class="form-control strip-tags resetElement" id="TaskID" required>
                                <option value="">--Select--</option>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="col-xs-12 col-md-12 input-group">
                            {!! Form::date('AssesmentDate',
                                ...
                                array('class'=>'form-control strip-tags', 'id'=>'AssesmentDate'))
                            !!}
                        </div>
                    </div>
...
<script>
    ...
    function calculateAnsScore(ele)
    {
        ...
        //assign 0 where values are empty string
        if(tskTime == ''){
            tskTime = 0;
        }
        if(tskSequence == ''){
        ...
        //count disabled cell according to row
        var anableCnt = 0;
        var totalWetAmnt = 0;
        if(tskTimeRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskTimeRow);
        }
        if(tskSequenceRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskSequenceRow);
        }
        if(tskQualityRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskQualityRow);
        }
        if(tskQuantityRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskQuantityRow);
        }
        if(tskDeliveryRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskDeliveryRow);
        }
        if(tskTimetakenRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskTimetakenRow);
        }
        if(tskTargetRow > 0){
            anableCnt++;
            totalWetAmnt += Number(tskTargetRow);
        }
        //end of count
        // alert('Anable Count: '+anableCnt);
        // alert('Total Wet Amount: '+totalWetAmnt);
        var remainAmount = (35 - anableCnt * 5)/totalWetAmnt;
        // alert('Remain Amount: '+remainAmount);
        // alert('Task Time: '+tskTime);
        if(tskTimeRow == 0){
            var tskTimeTotal = 0;
        }else{
            //var tskTimeTotal = (Number(tskTime)*(Number(tskTimeRow)+(Number(tskTimeRow)*remainAmount)))/tskTimeRow;
            var tskTimeTotal = Number(tskTime)+((Number(tskTimeRow)*remainAmount)/5)*Number(tskTime);

        }
        // alert('Task Time Row: '+tskTimeRow);
        // alert('total task time: '+tskTimeTotal);
        if(tskSequenceRow == 0){
            var tskSequenceTotal = 0;
        }else{
            // var tskSequenceTotal = (Number(tskSequence)+((Number(tskSequenceRow)*remainAmount))/5)*Number(tskSequence);
            var tskSequenceTotal = Number(tskSequence)+((Number(tskSequenceRow)*remainAmount)/5)*Number(tskSequence);
        }
        if(tskQualityRow == 0){
            var tskQualityTotal = 0;
        }else{
            // var tskQualityTotal = (Number(tskQuality)+((Number(tskQualityRow)*remainAmount))/5)*Number(tskQuality);
            var tskQualityTotal = Number(tskQuality)+((Number(tskQualityRow)*remainAmount)/5)*Number(tskQuality);
        }
        if(tskQuantityRow == 0){
            var tskQuantityTotal = 0;
        }else{
            // var tskQuantityTotal = (Number(tskQuantity)+((Number(tskQuantityRow)*remainAmount))/5)*Number(tskQuantity);
            var tskQuantityTotal = Number(tskQuantity)+((Number(tskQuantityRow)*remainAmount)/5)*Number(tskQuantity);
        }
        if(tskDeliveryRow == 0){
            var tskDeliveryTotal = 0;
        }else{
            // var tskDeliveryTotal = (Number(tskDelivery)+((Number(tskDeliveryRow)*remainAmount))/5)*Number(tskDelivery);
            var tskDeliveryTotal = Number(tskDelivery)+((Number(tskDeliveryRow)*remainAmount)/5)*Number(tskDelivery);
        }
        if(tskTimetakenRow == 0){
            var tskTimetakenTotal = 0;
        }else{
            // var tskTimetakenTotal = (Number(tskTimetaken)+((Number(tskTimetakenRow)*remainAmount))/5)*Number(tskTimetaken);
            var tskTimetakenTotal = Number(tskTimetaken)+((Number(tskTimetakenRow)*remainAmount)/5)*Number(tskTimetaken);
        }
        if(tskTargetRow == 0){
            var tskTargetTotal = 0;
        }else{
            // var tskTargetTotal = (Number(tskTarget)+((Number(tskTargetRow)*remainAmount))/5)*Number(tskTarget);
            var tskTargetTotal = Number(tskTarget)+((Number(tskTargetRow)*remainAmount)/5)*Number(tskTarget);
        }

        var total = tskTimeTotal + tskSequenceTotal + tskQualityTotal + tskQuantityTotal + tskDeliveryTotal + tskTimetakenTotal + tskTargetTotal;

        // var maxTotal = 5*Number(tskTimeRow) + 5*Number(tskSequenceRow)
        //     + 5*Number(tskQualityRow) + 5*Number(tskQuantityRow)
        //     + 5*Number(tskDeliveryRow) + 5*Number(tskTimetakenRow)
        //     + 5*Number(tskTargetRow);
        var maxTotal = 35;
        $(trID +" .AnsScore").val(total);
        $(trID +" .maxScore").val(maxTotal);

        // Step 1-	(35-number of eiligible item*5)/total weight of eligible Item
        // Step -2	Weight*Step1 Value= new wait
        // Step 3: 	Step 2* given marks
        // Step 4:	step 3/actual weight
